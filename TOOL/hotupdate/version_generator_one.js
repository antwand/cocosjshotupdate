...
while ( i < process.argv.length) {
    ...
    case '-u' :
        ...
        manifest.remoteManifestUrl = url + dest + 'project.manifest';
        manifest.remoteVersionUrl = url +dest +  'version.manifest';
        i += 2;
        break;
    case '--version' :
    case '-v' :
...
function readDir (dir, obj) {
    ...
    for (var i = 0; i < subpaths.length; ++i) {
        ...
        if (game_name.length >2){
            ...
            if(
                subpath.indexOf(path.join(src, 'src/game/' + game_name +"/")) >= 0 ||
                subpath.indexOf(path.join(src, 'res/game/' + game_name + "/")) >= 0
            ){
